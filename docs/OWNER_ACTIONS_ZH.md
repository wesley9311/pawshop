...
| # | 事项 | 状态 | 只剩你要做的动作 |
| --- | --- | --- | --- |
| 1 | **告警通道**（P0） | ⚠️ **飞书曾静默失效**（关键词被改）→ 已定位并修好，**待随下次发版生效**；Slack 全程正常 | 无；但**以后改飞书关键词请说一声**（§1.3） |
...
## 1. 告警通道（唯一"出事没人知道"的阻塞项）— 已修复，待随下次发版生效

**结论**：监控会**同时**把告警发到飞书和 Slack，任一条被对方确认即算送达。

**但今天（2026-09-17 晚）发现飞书这一条其实一直是哑的**：你改了机器人里的"自定义关键词"，而我们的消息前缀没跟着变，飞书于是把**每一条**告警都拒收了（拒收时返回的是 **HTTP 200**，只看状态码会以为发成功）。**这段时间里真正能叫醒你的只有 Slack。** 原因已定位，代码已按你的关键词适配（发版后生效，见 §1.1）。Slack 全程正常，所以"出事完全没人知道"这件事**没有发生过**。

### 1.1 我实测到的三件事（都与你的设置有关）

1. **你给的两条 URL 都有效**，从生产主机（美国西部）直连即可送达，不需要额外网络处理。
2. **你的飞书机器人加了"自定义关键词"过滤，关键词现在是 `[PawShop]`——方括号在内、里面没有空格、也没有"告警"两个字。** 过滤是**按消息正文匹配**的（子串命中即通过），所以任何"不含 `[PawShop]` 这一段"的消息会被飞书**拒收**，并且飞书拒收时返回 **HTTP 200 + `code:19024 Key Words Not Found`**。
3. **这个关键词被改过**：今天上午实测时它还是 `[PawShop 告警]`，晚上就只认 `[PawShop]` 了。**`[PawShop 告警]` 里并不含 `[PawShop]` 这个子串**（差一个 `]` 的位置），所以前缀一字之差 = 全部告警静默丢失。这就是上面那条哑掉的直接原因。

   > **你要做的：什么都不用改。** 我把消息前缀改成了 `[PawShop] 告警`——它**同时**含 `[PawShop]` 和 `告警`，所以**你现在这个关键词、以及以后可能改成的 `告警` 都能命中**。并加了回归测试锁住这条不变量（两种消息前缀必须相同，且逐字包含 `PawShop` / `[PawShop]` / `告警`）。
   >
   > ⚠️ 唯一需要你配合的：**以后如果又去改这个关键词，改完告诉我一声。** 群里改设置不会通知主机，而"发不出去"在日志里只表现为一条通道静默失败。

### 1.2 投递验证的证据（不是"跑通了"，是真送达）

```
INFO alert channels configured: feishu, slack
INFO alert channel feishu accepted the payload      ← 飞书真收了（用改后的前缀重测）
...
今晚的修复验证是**直接用真实飞书机器人与真实 Slack webhook** 跑的：先造一次失败发告警，再恢复正常发恢复通知，**两条消息四个渠道全部 `accepted`**。验证用的状态文件与生产状态文件是分开的，跑完即删，**没有污染生产的告警抑制状态**。

同一次排查还顺手补上了一个短板：以前通道被拒时日志只说"没接受"，不说**为什么**。现在会写成 `provider code 19024: … this bot requires a keyword …`，**下次再出现类似情况，看一眼日志就知道要去改哪里**，不必再临时写探针。
...
- 🔑 关键词怎么填才不会再出今天这种事：**只要能当 `[PawShop] 告警` 的子串就一定两边都通**。最省事的两个选择：`[PawShop]`（你现在这个，不用动）或 `告警`。**不要**填 `PawShop 告警` 这种中间带空格的写法——它不在信封里。
- 📋 **改完关键词请说一声**，我 1 分钟复跑一次验证。否则下一次"告警发不出去"很可能要等到真出事才被发现。
