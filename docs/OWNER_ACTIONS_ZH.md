...
| # | 事项 | 状态 | 只剩你要做的动作 |
| --- | --- | --- | --- |
| 0 | **后台账号 + 忘记密码** | ✅ 账号已建好、真实登录验证通过；✅ 忘记密码链路已打通，**只差一个发信邮箱** | 给我 QQ 邮箱的 **SMTP 授权码**（§0.1，2 分钟） |
| 1 | **告警通道**（P0） | ✅ **两条都通了**（2026-09-18 实测：告警 ×2 + 恢复 ×2 全部被接受）；**两个 webhook 已轮换**，旧地址作废 | 无（已结） |
| 2 | **备份链**（P0） | ✅ **凭据已建、已实测、四项阻塞全清**；首次备份与恢复演练已随本次发版跑通 | 无 |
| 3 | OSS 生命周期三条规则 | ✅ **已配置并逐条核对**（90/365/1095 天） | 无 |
| 4 | **临时 RAM 用户清理** | ⚠️ **只剩这个**：我删不掉，需要你点两下 | 删除 `pawshop-agent-temp`（§2.3） |
| 5 | **推送待批准的提交** | ✅ 已全部推送（含今晚的告警修复与"忘记密码"功能） | 无 |
...
> **现在真正卡在你手上的只有两件小事**：给我 QQ 邮箱的 SMTP 授权码（§0.1，2 分钟，让"忘记密码"从假功能变成真功能）、删一个已经作废的临时用户（§2.3，1 分钟）。告警通道那件**已经结掉了**——两条都通、webhook 已轮换（§1 + §1.5）。其余我都能自己跑完。
...
## 1. 告警通道 — ✅ 两条都通了（2026-09-18 轮换后实测）

**现状（2026-09-18 上午，刚实测）**：**飞书与 Slack 都确认接收**。你重建飞书机器人之后，我复跑了完整投递验证，**四条消息（告警 ×2 通道 + 恢复 ×2 通道）全部被对方接受**，日志逐条为证：

```
INFO  alert channel feishu accepted the payload      ← 飞书，之前一直回 19024 拒收
INFO  alert channel slack accepted the payload
ERROR monitoring failed 1/12 checks: tls_certificate  ← 故意造的故障，用来逼出一次真告警
...
顺带把两个 webhook 都**轮换**了一遍（新地址就位、旧地址作废，见 §1.5）——并且确认了**当前有效的凭据只存在于服务器上那一个文件里**。

**"出事完全没人知道"这件事从头到尾没有发生过**：Slack 那条从接入到现在一直是通的。监控同时发两条通道，任一条被确认即算送达——两条都活着，意味着"某家平台抽风或某个群被删"只会损失一条，而不是丢掉整个告警。

> §1.1–§1.3 是当时那三次漂移的**诊断记录**，问题已解决、留作参考：万一飞书这条将来又哑了，按那里的顺序定性，不要凭猜。
