...
| 1 | **告警通道**（P0） | ⚠️ **Slack 正常；飞书仍被拒**——你的关键词一天内变了三次，最后一次猜不到 | **把飞书机器人安全设置改成"IP 白名单"并填 `47.254.26.124`**（§1.2，1 分钟） |
...
> **现在真正卡在你手上的只有三件小事**：给我 QQ 邮箱的 SMTP 授权码（§0.1，2 分钟，让"忘记密码"从假功能变成真功能）、点两下改飞书的安全设置（§1.2，1 分钟，让告警真的能叫醒你）、删一个已经作废的临时用户（§2.3，1 分钟）。其余我都能自己跑完。
...
- 🔑 若坚持用关键词：**只要能当 `[PawShop] 告警` 的子串就一定两边都通**——`[PawShop]` ✓、`告警` ✓、`PawShop` ✓；`PawShop 告警`（中间带空格）✗、`【PawShop】`（全角括号）✗。
