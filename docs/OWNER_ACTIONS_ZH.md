...
| # | 事项 | 状态 | 只剩你要做的动作 |
| --- | --- | --- | --- |
| 0 | **后台账号 + 忘记密码** | ✅ 账号已建好、真实登录验证通过（**2026-09-18 发版后重新验过一次，数据没丢**）；✅ 忘记密码链路已打通，**只差一个发信邮箱** | 给我 QQ 邮箱的 **SMTP 授权码**（§0.1，2 分钟） |
| 1 | **告警通道**（P0） | ⚠️ **飞书 ✅ / Slack ❌**（2026-09-18 实测：飞书告警与恢复均被接受；**Slack 返回 404，那个 webhook 已经不存在了**）。现在是"1/2 通道"，告警仍能到你手上，但少了一条冗余 | 重新生成 **Slack Incoming Webhook URL**（§1.6，1 分钟） |
...
> **卡在你手上的只有三件小事**：① 给我 QQ 邮箱的 SMTP 授权码（§0.1，2 分钟，让"忘记密码"从假功能变成真功能）；② 重新生成 Slack webhook URL（§1.6，1 分钟，把告警恢复到双通道）；③ 删掉那个已经作废的临时用户（§2.3，1 分钟）。**商务后台的工程侧已经全部通了**——升级路径首次实跑成功，你的后台账号和库里的数据一个字都没丢。
...
**只剩一步：给它一个真的发信通道。** 你有 QQ 邮箱，最省事的就是用它：

1. 打开 <https://mail.qq.com> → **设置** → **账户** → 找到 **"IMAP/SMTP服务"** → **开启** → 按提示用手机发一条短信 → 得到一串 **16 位授权码**。
2. 把这串**授权码**给我（**不是你的 QQ 密码**；授权码只能用来发信、登录不了你的邮箱，随时可以作废重发）。两种交接方式，挑你顺手的：
   - **最快（推荐，和这次 webhook 一样）**：桌面上新建一个文本文件（例如 `qq-smtp.txt`），里面**只放那 16 位授权码**，跟我说一句"放好了"。我读出后**删掉这个文件**——它不会进聊天记录。
   - **剪贴板**：复制后说一句"复制好了"，我读取剪贴板并**读完立刻清空**。
3. 剩下的全是我做（约 2 分钟，一条命令），顺序是刻意的——**先证明能发信，才允许写进服务器**：
