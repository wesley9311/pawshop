...
### 1.4 URL 怎么安全交给我（三选一，别贴聊天）

Webhook URL 等同于一个"能往你频道里发消息"的写入凭据。看到它的人就能冒充你的运维告警。所以：

- **推荐**：把它存成一个本地文件，例如 `~/pawshop-alert-webhook.txt`（一行，只有 URL），然后跟我说"存好了"。我读出来、传到服务器、写入配置，**之后你自己把这个文件删掉**。
- 或者：你 SSH 上服务器自己交互式写入（我给你逐字命令，用 `read -s` 避免进 shell 历史）。
