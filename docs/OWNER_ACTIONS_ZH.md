...
2. 把这串**授权码**给我（**不是你的 QQ 密码**；授权码只能用来发信、登录不了你的邮箱，随时可以作废重发）。三种交接方式，挑你顺手的：
   ...
   - **你自己在服务器上写**（想完全绕开我也行）——在**生产主机**上以 root 执行：
     ```bash
     read -rsp "QQ 邮箱授权码（输入时不显示）: " P; echo
     python3 -c 'import json,sys;d={"from":"504533680@qq.com","host":"smtp.qq.com","password":sys.argv[1],"port":465,"secure":True,"user":"504533680@qq.com"};open("/etc/pawshop/email-credentials.json","w").write(json.dumps(d)+"\n")' "$P"
     chown root:pawshop /etc/pawshop/email-credentials.json && chmod 0640 /etc/pawshop/email-credentials.json
     unset P && systemctl restart pawshop-commerce.service
     ```
     文件权限必须是 `root:pawshop 0640`（或 `root:root 0600`）、不能是符号链接——服务启动时会**复核**这些，不合格会**拒绝启动**而不是睁一只眼闭一只眼。
3. 我写进服务器（root-only 文件，不进 `git`、不进环境变量），2 分钟内跑一次真实发信验证，你会收到一封测试邮件。
