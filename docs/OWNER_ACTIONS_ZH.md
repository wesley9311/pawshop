...
| 1 | **告警通道**（P0） | ⚠️ **Slack 正常；飞书仍被拒**——你的关键词一天内变了三次，最后一次猜不到。另外调试时那条 URL 被打印进过会话，**建议顺手轮换一次** | **删掉旧飞书机器人、重建一个并把安全设置选成"IP 白名单"填 `47.254.26.124`**（§1.2 + §1.5，2 分钟） |
...
> **现在真正卡在你手上的只有三件小事**：给我 QQ 邮箱的 SMTP 授权码（§0.1，2 分钟，让"忘记密码"从假功能变成真功能）、删掉旧飞书机器人重建一次并把安全设置选成 IP 白名单（§1.2 + §1.5，2 分钟，让告警真的能叫醒你、同时作废那条被我打印过的 URL）、删一个已经作废的临时用户（§2.3，1 分钟）。其余我都能自己跑完。
...
2. 把这串**授权码**给我（**不是你的 QQ 密码**；授权码只能用来发信，登录不了你的邮箱，随时可以作废重发）。
   - 更稳妥（推荐）：**你自己在服务器上**执行我给你的那条命令，交互式输入授权码——这样它完全不经过聊天记录。
...
## 1. 告警通道 — Slack 正常；飞书需要你点两下（建议改 IP 白名单）

**现状（2026-09-17 晚，刚实测）**：**Slack 正常**，**飞书当前仍在拒收**——需要你点两下（见下）。

监控会**同时**把告警发给飞书和 Slack，任意一条被对方确认即算送达。所以"出事完全没人知道"这件事**没有发生过**：Slack 这条从接入到现在一直是通的。
...
### 1.2 ✅ 推荐做法（1 分钟，治本）：把过滤方式换成 IP 白名单
