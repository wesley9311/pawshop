...
所以现在信封取 `[PawShop] 告警`，**同时**含 `[PawShop]` 与 `告警`——店主**已存在的**关键词 `[PawShop]` 和可能的 `告警` 都能命中，无需改飞书设置。`_commerce/tests/monitoring.test.cjs` 有三条断言锁住它：两种消息前缀相同、且逐字包含 `PawShop` / `[PawShop]` / `告警`。
...
**判定**：命中 → 把它写进 §9.3 的信封不变量（信封必须**逐字包含**它），并补一条断言进 `monitoring.test.cjs`。全部未命中 → **不要继续猜**，直接问店主当前关键词（他改的，他知道），或请他把关键词设成信封里的 `[PawShop]`。
