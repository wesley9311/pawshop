...
**消息信封是一条不变量，不要改**（2026-09-17 两次实测教训）
...
原因：飞书自定义机器人的"自定义关键词"过滤是**按消息正文匹配**的，未命中时**飞书返回 HTTP 200 + `code:19024 Key Words Not Found`**，只看状态码会判成"已投递"。

**这里踩过两次，两次都是静默的**：

1. 恢复通知原来以 `[PawShop 恢复]` 开头 → 不对称：告警送得到、恢复送不到。
2. 信封改成统一的 `[PawShop 告警]` 之后，**线上群的关键词当天又变成了 `[PawShop]`（含方括号、无空格）**——`[PawShop 告警]` 里并不含子串 `[PawShop]`，于是**所有**告警被拒，而监控一直报 12/12 全绿。当晚只靠 Slack 一条通道。发现方式：日志里那条 `alert channel feishu did not accept the payload`（当时还只有状态码，没有错误码，见下）。
3. **同一天傍晚第三次漂移**：信封已改成 `[PawShop] 告警`（逐字包含 `[PawShop]`），这条也在真实投递时被 `19024` 拒收；随后用探针枚举 18 个候选（`PawShop`、`[PawShop]`、`告警`、`监控`、`服务器`、`pawlivora` …）**全部被拒**——而两小时前 `[PawShop]` 还是通的。群侧关键词在几小时内变了三次，最后一次没有任何可猜的线索。

**⚠️ 结论：靠"让代码去适配一个随时会变的群侧关键词"治不好这个病。** 关键词是群侧资产、改动不通知主机，所以每次改文案或改设置都可能让告警静默消失，而监控照样全绿。稳的做法是**把飞书机器人的安全设置从"自定义关键词"换成"IP 白名单"，填生产主机出口 IP `47.254.26.124`**：与消息正文彻底解耦（以后改文案零风险），而且比关键词更强（只有这台主机能发）。在这一步完成前，**Slack 是唯一可靠的出口**；代码侧继续用 `[PawShop] 告警` 信封（对任何"从信封切出来的关键词"都是良好默认，换了也不亏）。

所以现在信封取 `[PawShop] 告警`，**同时**含 `[PawShop]` 与 `告警`。`_commerce/tests/monitoring.test.cjs` 有三条断言锁住它：两种消息前缀相同、且逐字包含 `PawShop` / `[PawShop]` / `告警`。

**关键词是群侧资产，随时可能被改，且改的时候主机不会收到任何通知。** 怀疑告警没到，按下面顺序查（不要凭猜）：

```bash
# 1) 日志里的错误码已经直接说明原因（19024 = 关键词不匹配，19021 = 开了签名校验）
journalctl -u pawshop-monitor.service -n 40 --no-cache --no-pager -o cat | grep -E "alert channel|alert reached"

# 2) 仍未定性时，用只读探针枚举候选关键词。被拒的消息飞书根本不会投递，
#    所以探测不会刷屏；一旦命中才有一条消息进群。
#    见 §9.3.1「关键词探针」。
```
...
```bash
cd _commerce
npm run test:alert-delivery     # 16 项判定全绿；把四家的报文逐条打到收端上核对
```

该夹具会起一个真 HTTPS 接收端，按四家的真实应答（含"200 + 错误码"陷阱，含 2026-09-17 线上那条 `19024`）回包，并用 `dns-stub.mjs` 只把厂商域名解析到本地，**webhook URL 仍保留真实域名**，所以域名钉住策略照样生效。覆盖场景：单通道四家方言与判定、**双通道全通 / 一通一死 / 全死 / 飞书陷阱被另一条遮住**、抑制窗口、关键词拒收时日志必须点名错误码，以及"两种写法同时出现必须拒绝启动"。它证明的是"方言与判定正确"，**不**证明"你的通道存在"——后者要靠下面的真实验证。
...
**判定标准**：日志出现 `alert channel <label> accepted the payload`（每条通道各一行）**且**对应频道里真的看到那条消息——两者缺一都不算通过。若出现 `did not accept the payload`，日志里现在**同时给出厂商错误码与含义**（`provider code 19024: … this bot requires a keyword …`），按码定位，不要凭猜。
...
### 9.3.1 关键词探针：群侧关键词被改后如何定性（2026-09-17 实测）

**为什么需要它**：关键词是**群里的设置**，主机读不到、也收不到变更通知。日志里的 `19024` 只说明"没命中"，不说明"应该命中什么"。凭猜改代码会浪费一整轮发版。

**关键前提**：**未命中关键词的消息飞书根本不会投递**，所以枚举候选词**不会刷屏**——只有命中的那一次才会有一条消息进群（正文自带 `self-check` 字样，可忽略）。

**做法**（主机 root，只读环境文件，探针脚本用完即删）：

1. 用**已安装的** `monitoring-policy.cjs` 解析出 channel 列表，保证与监控的解析完全一致（含域名钉住）。
2. 逐候选发一条 `PawShop self-check <候选> (ops script, safe to ignore)`，用 `alertDeliveryAccepted` 判定，命中即停。
3. 输出里**把 URL 的 token 段替换成 `<redacted>`** 再打印，只回传 `status` 与 `code`（`msg` 也可能回显请求细节，截断）。

**候选词怎么排**：先排"从现有信封能切出来的片段"（`[PawShop]`、`PawShop 告警`、`告警`、`PawShop`），再排常见运维词（`监控`、`告警通知`、`服务器告警`、`生产环境`…）与域名（`pawlivora.com`）。**注意全角/半角括号与空格**：`【PawShop 告警】`、`[PawShop 告警]`、`[PawShop]` 是三件不同的事，实测只有最后一个被接受。

**判定**：命中 → 把它写进 §9.3 的信封不变量（信封必须**逐字包含**它），并补一条断言进 `monitoring.test.cjs`。全部未命中 → **不要继续猜**，也不要再改代码去迁就：**请店主把安全设置从"自定义关键词"改成"IP 白名单"并填 `47.254.26.124`（推荐，治本）**；他若坚持用关键词，就请他直接把当前关键词告一知，然后把它并进信封。
