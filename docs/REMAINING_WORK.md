...
4. **Airwallex / PingPong 收款申请**、**后台对店主的暴露方式**（SSH 隧道 vs 自建中文运营台）—— ⚠️ **这是当前挂在店主这一侧的仅剩两项，都是产品决策，不阻塞任何工程动作。**

**我这一侧唯一未落地的动作**：本地提交 `a1b4931`（纯文档：凭据轮换记录、备份密钥为何不轮换、登录取证结论、RUNBOOK §13 凭据纪律）**尚未推送**——按"推送前先问"的约定在等店主点头，且本机代理 `127.0.0.1:7892` 此刻未监听。
...
| A8 | `sshd` 多开 22222 端口 + 无用的 `Match User admin` 密码登录块 | P2 | **共同** | `/etc/ssh/sshd_config.d/99-pawshop.conf` 里有 `Port 22` + `Port 22222`，而**仓库任何脚本都不含 22222** → 是主机上手工加的、无文档。另有一块 `Match User admin / PasswordAuthentication yes`；`admin` 密码是**锁定**状态（`passwd -S` = `L`），故**无可利用口子**，但属残留。**需店主确认 22222 是否在用**：不用则可摘除（会先备份、`sshd -t` 校验、**保留 22 不动**，不会锁人）；在用则写成正式约定。**不擅自关**——关错端口会把店主锁在门外。 |
