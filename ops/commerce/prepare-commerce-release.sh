...
resolved_tree=$(git_readonly rev-parse --verify "$PAWSHOP_RELEASE_ID^{tree}")
...
git_readonly archive --format=tar --output="$archive_file" "$PAWSHOP_RELEASE_ID" -- _commerce ops/commerce
...
/usr/bin/node "$source_dir/_commerce/scripts/create-release-manifest.mjs" \
  "$staging_dir" "$PAWSHOP_RELEASE_ID" "$resolved_tree"
chmod 0644 "$staging_dir/.pawshop-release.json"
/usr/bin/node "$source_dir/_commerce/scripts/verify-release-manifest.mjs" \
  "$staging_dir" "$PAWSHOP_RELEASE_ID" >/dev/null
...
if [[ ! -d $release_dir || -L $release_dir || ! -f $release_dir/.pawshop-release ||
   ...
   [[ $(stat -c '%u:%g:%a' -- "$release_dir/.pawshop-release.json") != '0:0:644' ]] ||
...
/usr/bin/node "$source_dir/_commerce/scripts/verify-release-manifest.mjs" \
  "$release_dir" "$PAWSHOP_RELEASE_ID" >/dev/null
